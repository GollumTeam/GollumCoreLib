...
public abstract class ConfigEntry implements IGuiListEntry {
	...
	public ConfigEntry(int index, Minecraft mc, GuiConfigEntries parent, ConfigElement configElement) {
		...
		this.toolTip.add (TextFormatting.GREEN + configElement.getName());
		if (!prop.info().equals("")) {
			this.toolTip.add (TextFormatting.YELLOW + prop.info());
		}
		if (!prop.minValue().equals("") || !prop.minValue().equals("")) {
			this.toolTip.add (TextFormatting.AQUA + I18n.format("fml.configgui.tooltip.defaultNumeric", configElement.getMin(), configElement.getMax(), defaultValueStr));
		} else {
			this.toolTip.add (TextFormatting.AQUA + I18n.format("fml.configgui.tooltip.default", defaultValueStr));
		}
		...
		if (prop.worldRestart()) {
			toolTip.add(TextFormatting.RED + "[" + ModGollumCoreLib.i18n.trans("config.worldRestartTitle") + "]");
		}
	...
	public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected, float partial) {
		this.drawEntry(slotIndex, x, y, listWidth, slotHeight, mouseX, mouseY, isSelected, true, partial);
	}
	...
	public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected, boolean resetControlWidth, float partial) {
		...
		if (this.getLabelDisplay()) {
			...
			if (this.formatedLabel) {
				label = (!this.isValidValue() ? TextFormatting.RED.toString() : (this.isChanged() ? TextFormatting.WHITE.toString() : TextFormatting.GRAY.toString())) + (this.isChanged() ? TextFormatting.ITALIC.toString() : "") + label;
			}
		...
		if (this.parent.parent.canRemove ()) {
			this.btnRemove.x = this.parent.scrollBarX - posArBt;
			this.btnRemove.y = y;
		...
		if (this.parent.parent.canAdd ()) {
			this.btnAdd.x = this.parent.scrollBarX - posArBt;
			this.btnAdd.y = y;
		...
		this.btUndo.x = this.parent.scrollBarX - 44;
		this.btUndo.y = y;
		this.btUndo.enabled = this.enabled() && this.isChanged() && this.parent.parent.undoIsVisible();
		this.btUndo.drawButton(this.mc, mouseX, mouseY, partial);
		
		this.btReset.x = this.parent.scrollBarX - 22;
		this.btReset.y = y;
		this.btReset.enabled = this.enabled() && !this.isDefault() && this.parent.parent.resetIsVisible();
		this.btReset.drawButton(this.mc, mouseX, mouseY, partial);
