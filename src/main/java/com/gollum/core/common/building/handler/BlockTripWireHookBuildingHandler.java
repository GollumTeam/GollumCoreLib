...
public class BlockTripWireHookBuildingHandler extends BuildingBlockHandler {
	...
	protected boolean mustApply (World world, int x, int y, int z, Unity unity) {
		return 
			unity.block instanceof BlockTripWireSource
		;
	}
	...
	protected int applyMetadata(World world, int x, int y, int z, int metadata, Unity unity, int rotate) {
		...
		if (orientation == Unity.ORIENTATION_DOWN)  { metadata = (metadata & 0xC) + 0; } else 
		if (orientation == Unity.ORIENTATION_LEFT)  { metadata = (metadata & 0xC) + 1; } else 
