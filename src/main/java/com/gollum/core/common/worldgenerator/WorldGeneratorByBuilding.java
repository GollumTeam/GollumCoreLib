...
public class WorldGeneratorByBuilding implements IWorldGenerator {
	...
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
		...
		int dimention = world.provider.getDimension();
	...
	private ArrayList<Building> filterByBiome(Biome biome, ArrayList<Building> buildings, int dimention) {
	...
	private boolean generateBuilding(World world, Random random, int chunkX, int chunkZ, ArrayList<Building> buildings, int groupSpawnRate, int dimention) {
		...
		buildings = this.filterByBiome(world.getBiome(new BlockPos(chunkX*16, 1, chunkZ*16)), buildings, dimention);
						...
						if (dimentionsInfos.spawnMax >= dimentionsInfos.spawnMin) {
							...
							for (; initY >= dimentionsInfos.spawnMin;initY--) {
								...
								if (
									...
									BuildingGenerateEvent event = new BuildingGenerateEvent.Pre(world, building, rotate, new BlockPos(initX, initY, initZ));
