package mods.gollum.core.worldgenerator;
...
import mods.gollum.core.building.Building.Unity;
import mods.gollum.core.building.Building.Unity.Content;
...
//				initY = 3; // Pour test sur un superflat
				...
				if (
					world.getBlock(initX + 3, initY, initZ + 3) == Blocks.grass
				) {
					...
					for (int x = 0; x < building.maxX; x++) {
						for (int y = 0; y < building.maxY; y++) {
							for (int z = 0; z < building.maxZ; z++) {
									...
									int finalY = initY + y;
									int finalZ = initZ + z;
									world.setBlock(finalX, finalY, finalZ, Blocks.grass, 0, 2);
					...
					for (int x = 0; x < building.maxX; x++) {
						for (int y = 0; y < building.maxY; y++) {
							for (int z = 0; z < building.maxZ; z++) {
								...
								if (unity.block != null) {
									world.setBlock(finalX, finalY, finalZ, unity.block, unity.metadata, 2);
								} else {
						...
						for (int x = 0; x < lisBlockRandom.maxX; x++) {
							for (int y = 0; y < lisBlockRandom.maxY; y++) {
								for (int z = 0; z < lisBlockRandom.maxZ; z++) {
									...
									if (unity.block != null && unity.block == Blocks.air) {
										world.setBlock(finalX, finalY, finalZ, unity.block, unity.metadata, 2);
	...
	private boolean placeUnderBlock (World world, int finalX, int finalY, int finalZ, int profondeur) {
		if (
			world.getBlock(finalX, finalY, finalZ) != Blocks.grass   &&
			world.getBlock(finalX, finalY, finalZ) != Blocks.stone   &&
			world.getBlock(finalX, finalY, finalZ) != Blocks.dirt    &&
			world.getBlock(finalX, finalY, finalZ) != Blocks.bedrock &&
			finalY > 0
		) {
			if (profondeur > -5) {
				world.setBlock(finalX, finalY, finalZ, Blocks.grass, 0, 2);
			} else {
				world.setBlock(finalX, finalY, finalZ, Blocks.stone, 0, 2);
			}
			return true;
		}
			...
			if (te instanceof IInventory) {
				...
				for (int i = 0; i < contents.size(); i++) {
					...
					if (content.type == Content.TYPE_ITEM) {
						...
						if (content.metadata == -1) {
							itemStack = new ItemStack(content.block, nombre);
						} else {
							itemStack = new ItemStack(content.block, nombre, content.metadata);
						}
					}
